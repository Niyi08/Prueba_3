CREATE TABLE estudiante.md_derivatives (
  "ID" NUMBER NOT NULL,
  ...
  CONSTRAINT md_derivatives_md_connect_fk1 FOREIGN KEY (derived_connection_id_fk) REFERENCES estudiante.md_connections ("ID") ON DELETE CASCADE
);
COMMENT ON TABLE estudiante.md_derivatives IS 'This table is used to store objects that are derived from each other.  For example in a migration an auto-increment column in a source model could be mapped to a primary key, and a sequence, and a trigger.  The MD_DERIVATIVES table would store the fact that these 3 objects are derived from the auto-increment column.';
COMMENT ON COLUMN estudiante.md_derivatives.transformed IS 'Set this field to ''Y'' if we carry out any sort of transformation on teh derived object.';